...
def animate_big_cycle(placeholder, storage_left, storage_right, steps=60):
    # Temporarily empty storage for animation to show empty stockpoints during lift
    original_storage_left = storage_left
    original_storage_right = storage_right
    st.session_state.storage_left = 0
    st.session_state.storage_right = 0
    ...
    start_lift_y = -50
    end_lift_y = 50
    num_stored_left = original_storage_left // 10
    num_stored_right = original_storage_right // 10
    for step in range(steps):
        if st.session_state.stop_requested:
        ...
        placeholder.plotly_chart(fig, use_container_width=True)
        time.sleep(FRAME_DELAY)
    st.session_state.logs.append(f"Completed simultaneous drop 160kg and parallel lift {original_storage_left}kg from C, {original_storage_right}kg from D")
...
if st.session_state.running and not st.session_state.stop_requested:
    ...
    try:
        ...
        if total_storage >= STORAGE_THRESHOLD:
            # Save original storage for animation
            original_storage_left = st.session_state.storage_left
            original_storage_right = st.session_state.storage_right
            # Empty storage before animation
            st.session_state.storage_left = 0
            st.session_state.storage_right = 0
            st.session_state.logs.append(f"Action: Big cycle triggered (Storage = {total_storage}kg). Dropping 160kg...")
            ok = animate_big_cycle(scene_ph, original_storage_left, original_storage_right, steps=60)
            if not ok:
                st.session_state.stop_requested = True
            ...
            total_blocks_to_distribute = (original_storage_left + original_storage_right) // 10
            ...
            st.session_state.logs.append(
                ...
                f"Emptied storage points C and D. Used {(80_000 / B2_CAPACITY) * 100:.1f}% B2 to lift 160kg."
            )
            st.session_state.logs = st.session_state.logs[-100:]
            st.session_state.houses_lit = st.session_state.battery1 >= 10
            scene_ph.plotly_chart(draw_scene(), use_container_width=True)
